...
def read_buffer (
    ...
    check(cl.clEnqueueReadBuffer(
        command_queue,
        buffer,
        True, # This is a blocking operation
        0,
        size * 4, # again, assumes fp32. Needs to be changed for different fp types
    ...
    # ensure that we wait for the event (ensures blocking even further)
    # Although, this is not necessarily needed
    check(cl.clWaitForEvents(1, ctypes.pointer(event)))

    # convert pointer to np.array
    arr = np.ctypeslib.as_array(pointer, shape=(size,) if shape is None else shape)
    return arr
